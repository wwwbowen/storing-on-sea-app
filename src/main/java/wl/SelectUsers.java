    ...
    public SelectUsers() {
        ...
        ok.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if(uname.length() < 2) {
                    JOptionPane.showMessageDialog(SelectUsers.this, "管理员名称需大于两字符！");
                    return;
                }
                if(upass.length() < 2) {
                    JOptionPane.showMessageDialog(SelectUsers.this, "密码长度需大于两字符！");
                    return;
                }
                if( !upass.equals(repass) ) {
                    JOptionPane.showMessageDialog(SelectUsers.this, "password and confirm password should be the same");
                    return;
                }
                //后面连接数据库
                try {
                    ...
                    if(result > 0){
                        JOptionPane.showMessageDialog(SelectUsers.this, "添加成功！");
                        SelectUsers.this.dispose();
                    } else {
                            JOptionPane.showMessageDialog(SelectUsers.this, "添加失败");
                    }
                    if(statement != null) {
                        statement.close();
                    }
