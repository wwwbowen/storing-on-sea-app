    ...
    public AlterAdmin() {
        ...
        ok.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if(uname.length() < 2) {
                    JOptionPane.showMessageDialog(AlterAdmin.this, "用户名必须大于两字符！");
                    return;
                }
                if(upass.length() < 2) {
                    JOptionPane.showMessageDialog(AlterAdmin.this, "密码长度必须大于两字符！");
                    return;
                }
                if(!usex.equals("female")&&!usex.equals("male")) {
                    JOptionPane.showMessageDialog(AlterAdmin.this, "请输入正确的性别格式（female或male）！");
                    return;
                }
                if(uage.length() < 1) {
                    JOptionPane.showMessageDialog(AlterAdmin.this, "年龄不能为空！");
                    return;
                }
                if(utelephone.length() < 1) {
                    JOptionPane.showMessageDialog(AlterAdmin.this, "电话栏不能为空！");
                    return;
                }
                if(uaddress.length() < 1 ) {
                    JOptionPane.showMessageDialog(AlterAdmin.this, "地址栏不能为空！");
                    return;
                }
                //后面连接数据库
                try {
                    ...
                    statement = con.prepareStatement("update users set user_pass = ? , sex = ? , age = ? , telephone =? , address = ? where user_pass = ? and rid = 2");
                    ...
                    if(result > 0){
                        JOptionPane.showMessageDialog(AlterAdmin.this, "修改成功！");
                        AlterAdmin.this.dispose();
                    } else {
                            JOptionPane.showMessageDialog(AlterAdmin.this, "修改失败");
                    }
                    if(statement != null) {
                        statement.close();
                    }
